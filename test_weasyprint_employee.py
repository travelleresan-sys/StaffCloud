...
def create_employee_pdf_weasyprint(employee):
    """WeasyPrint版の従業員PDF生成"""
...
def test_weasyprint_employee():
    """WeasyPrint版従業員PDF生成をテスト"""
    with app.app_context():
        print("=== WeasyPrint従業員PDFテスト ===\n")
        
        employees = Employee.query.all()
        for employee in employees:
            try:
                print(f"従業員: {employee.name}")
                pdf_buffer = create_employee_pdf_weasyprint(employee)
                
                filename = f"weasyprint_employee_{employee.id}.pdf"
